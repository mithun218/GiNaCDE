...
\end{gathered}
	...
	\section{Arithmetic operators and some supported functions}
	...
	\section{The parameters of the different methods}
	...
	We have supplied the output files \emph{NLS{\_}Fexp.txt} and \emph{NLS\_FIM.txt} in the examples\ folder of GiNaCDE source code separately.
	...
	We have supplied the output file {\em KdVB\_mF.txt} in the examples\ folder of GiNaCDE source code.
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
    The complete C++ code is given in the examples/ folder of the GiNaCDE source code with a file name \emph{kerrNLS.cpp}.
	...
	We get exact solutions of the NLPDE \eqref{nlseWtKerr} with the F-expansion and mF-expansion methods. FIM is unable to solve the NLPDE \eqref{nlseWtKerr}, because the NLPDE \eqref{nlseWtKerr} in terms of traveling-wave coordinate $\xi$ is not a second-order NLODE. The output results obtained from running the above program have been provided under the examples/ folder of GiNaCDE source code with file names \textit{kerrNLS\_Fexp.txt}, \textit{kerrNLS\_mF.txt} and \textit{kerrNLS\_FIM.txt}.  
	
	
	\section{Non-Polynomial case in FIM}
		Now, we study the following Kudryashov–Sinelshchikov equation: %proposed in \cite{ks,ks1}:
	...
	The output results obtained from running the above program have been provided under the examples/ folder of GiNaCDE source code with file name \textit{KS\_FIM.txt}.
